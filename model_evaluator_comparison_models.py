# This file is used to evaluate PM and MnM models.
import os
...
# 1 - Predicting Matchability - PM (2014) paper - trained on their data (matches between images)
# 2 - Match or No Match: Keypoint Filtering based on Matching Probability - MoNM (2020) paper - trained on their data (matches between images)
# 3 - Predicting Matchability - PM (2014) paper - trained on my data (matches between images - 3D points)
# 4 - Match or No Match: Keypoint Filtering based on Matching Probability - MoNM (2020) paper - trained on my data (matches between images - 3D points)
...
# The directories should be already created from previous scripts, train_for_comparison_models_on_my_3D_data.py, create_training_data_predicting_matchability.py, create_training_data_for_match_no_match.py
...
comparison_data_path_PM_trained_my_3D_data = os.path.join(base_path, "predicting_matchability_comparison_data_trained_on_my_data")
comparison_data_path_MoNM_trained_on_my_3D_data = os.path.join(base_path, "match_or_no_match_comparison_data_trained_on_my_data")

if((os.path.exists(comparison_data_path_PM) == False) or (os.path.exists(comparison_data_path_MoNM) == False) or
    (os.path.exists(comparison_data_path_PM_trained_my_3D_data) == False) or (os.path.exists(comparison_data_path_MoNM_trained_on_my_3D_data) == False)):
    raise Exception("One of the dirs is not created!")

print("Loading Common Data.. for PM & MnM")
...
# TODO: Removed the models trained on my own 3D data for now as for MnM I will need to extract again the 8 features.
...
matches, images_matching_time, images_percentage_reduction = feature_matcher_wrapper_generic_comparison_model(base_path_mnm, comparison_data_path_MoNM, model, db_gt_mnm, localised_query_images_names_mnm, train_descriptors_live_mnm, points3D_xyz_live_mnm, ratio_test_val, model_type="MatchNoMatch")
...
# print("Getting matches using Match or No Match: Keypoint Filtering based on Matching Probability model trained on my 3D data + loading model..")
# model_path = os.path.join(comparison_data_path_MoNM_trained_on_my_3D_data, "rf_model.joblib")
# model = load(model_path)
# # for MnM it doesn't matter if you use "base_path" or "base_path_mnm" it just reads the same images (live).
# matches, images_matching_time, images_percentage_reduction = feature_matcher_wrapper_generic_comparison_model(base_path_mnm, comparison_data_path_MoNM_trained_on_my_3D_data, model, db_gt_mnm, localised_query_images_names_mnm, train_descriptors_live_mnm, points3D_xyz_live_mnm, ratio_test_val, model_type="MatchNoMatch")
# np.save(os.path.join(comparison_data_path_MoNM_trained_on_my_3D_data, "images_matching_time.npy"), images_matching_time)
# np.save(os.path.join(comparison_data_path_MoNM_trained_on_my_3D_data, "images_percentage_reduction.npy"), images_percentage_reduction)
#
# print(" RANSAC..")
# est_poses_results = benchmark(benchmarks_iters, ransac, matches, localised_query_images_names, K)
# np.save(os.path.join(comparison_data_path_MoNM_trained_on_my_3D_data, "est_poses_results.npy"), est_poses_results)
...
# print("Getting matches using Predicting Matchability (2014) model trained on my 3D data + loading model..")
# model_path = os.path.join(comparison_data_path_PM_trained_my_3D_data, "rf_model.joblib")
# model = load(model_path)
# matches, images_matching_time, images_percentage_reduction = feature_matcher_wrapper_generic_comparison_model(base_path, comparison_data_path_PM_trained_my_3D_data, model, db_gt, localised_query_images_names, train_descriptors_live, points3D_xyz_live, ratio_test_val)
# np.save(os.path.join(comparison_data_path_PM_trained_my_3D_data, "images_matching_time.npy"), images_matching_time)
# np.save(os.path.join(comparison_data_path_PM_trained_my_3D_data, "images_percentage_reduction.npy"), images_percentage_reduction)
#
# print(" RANSAC..")
# est_poses_results = benchmark(benchmarks_iters, ransac, matches, localised_query_images_names, K)
# np.save(os.path.join(comparison_data_path_PM_trained_my_3D_data, "est_poses_results.npy"), est_poses_results)
