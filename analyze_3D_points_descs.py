...
def analyze(points3D, db):
    ...
    for k,v in points3D.items():
        ...
        numbers_of_descs.append(len(points_image_ids))
    ...
    return np.array(numbers_of_descs)
...
nos_base = analyze(base_model_points3D, db_base)
# np.save(parameters.avg_descs_base_path, avgs_base)

nos_live = analyze(live_model_points3D, db_live)
# np.save(parameters.avg_descs_live_path, avgs_live)

N = nos_live.shape[0]
ind = np.arange(N)
width = 0.35
plt.bar(ind, nos_base, width, label='Base')
plt.bar(ind + width, nos_live, width, label='Live')

plt.ylabel('No. Descriptors')
plt.title('Number of descriptors per point')
