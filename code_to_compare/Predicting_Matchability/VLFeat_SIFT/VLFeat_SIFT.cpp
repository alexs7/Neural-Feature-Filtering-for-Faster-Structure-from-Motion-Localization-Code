...
char const help_message [] =
  ...
  " --classify -c   Reduce the number of non-matchable features\n"
  " --cl-thresh     Specify the classification threshold\n"
  "\n" ;
...
struct option const longopts [] = {
  ...
  { "cl-thresh",       required_argument,      0,          opt_cl_thresh    },
  { 0,                 0,                      0,          0                }
} ;
...
main(int argc, char **argv)
{
  ...
  vl_bool  force_classification = 0 ;
  ...
  VlFileMeta cls  = {0, "%",       VL_PROT_ASCII, "", 0} ;
...
#define ERR(msg) {                                              \
  ...
  while (!err) {
    ...
    case 'c' :
      /* --classify ...............................................*/
      err = vl_file_meta_parse (&cls, optarg) ;
      if (err)
        ERRF("The arguments of '%s' is invalid.", argv [optind - 1]) ;
      force_classification = 1 ;
      break ;

    case opt_cl_thresh :
      /* --cl-thresh .............................................*/
      n = sscanf (optarg, "%lf", &cl_thresh) ;
      if (n == 0 || cl_thresh < 0 || cl_thresh > 1)
        ERRF("The argument of '%s' must be a [0..1] float.",
            argv [optind - 1]) ;
      break ;
...
#define PRNFO(name,fm)                                                  \
  ...
  if (force_classification) {
      std::ifstream ifs(cls.pattern, std::ios_base::in | std::ios_base::binary) ;
      if (!ifs.is_open()) {
        err = VL_ERR_IO ;
        snprintf(err_msg, sizeof(err_msg),
               "Could not open '%s' for reading.", cls.pattern) ;
        fprintf (stderr, "sift: err: %s (%d)\n", err_msg, err) ;
        return 1 ;
      }
      if (verbose > 1) {
        if (out.active) printf("sift: read classifier from '%s'\n", cls.pattern) ;
      }
  ...
  if (verbose)
    printf ("sift: classification is %s\n",
            force_classification ? "enabled" : "disabled") ;
...
#define QERR                                                            \
    ...
    if (verbose > 1) {
      ...
      printf ("sift: will force classification? %s\n",
              force_classification ? "yes" : "no") ;
      if (force_classification)
        printf ("sift:   cl thresh             = %g\n", cl_thresh) ;
    }
    ...
    while (1) {
      ...
      for (; i < nkeys ; ++i) {
        ...
        for (q = 0 ; q < (unsigned) nangles ; ++q) {
          ...
          if (out.active || dsc.active || force_classification) {
