# Run this after create_universal_models.py and get_points_3D_mean_desc_ml_mnm.py
# You need the "all_images" folder for this script to work, from create_universal_models.py.
...
# To summarise:
# 1 - Run create_universal_models.py on CYENS machine (not needed for this file)
# 2 - get_points_3D_mean_desc_ml_mnm.py (not needed for this file)
...
# NOTE: 03/03/2023: This file does not depend on create_universal_models.py other than the "all_images" folder (which is created by create_universal_models.py)
# the ids of the images are the same in the live db and the mnm db, so we can use the live db to get the pairs. The live db is from ExMaps.
...
def createTrainingDataForMatchNoMatchMatchabilityComparison(mnm_path, original_live_db_path, all_images_path, mnm_base_code_dir, pairs_limit = -1, matchornomatch_tool=""):
    ...
    rows_no = 10
    print(f"Getting all pairs (rows > {rows_no}) total count, from the original live database..")
    ...
    if (pairs_limit == -1):
        ...
        pair_ids = original_live_db.execute(f"SELECT pair_id FROM two_view_geometries WHERE rows > 10 ORDER BY RANDOM() LIMIT {pairs_limit}").fetchall()
    ...
    matchornomatch_train = [f"./{matchornomatch_tool}", training_images_folder, training_data_folder, trained_model_name]
...
# For each dataset I use hte "all_images" folder, which contains all images from each dataset that was generated by create_universal_models.py
...
# In create_universal_models.py I am using the same pairs as in the original models
...
if(dataset == "HGE" or dataset == "CAB" or dataset == "LIN"):
    ...
    images_path = os.path.join(mnm_path, "all_images")  # all images from create_universal_models.py
    pairs_no = sys.argv[2]
    ...
    createTrainingDataForMatchNoMatchMatchabilityComparison(mnm_path, original_live_db_path, images_path, mnm_base_source_code_dir, pairs_limit, matchornomatch_tool="matchornomatch_train_2000_features_sift")
...
if(dataset == "CMU"):
    ...
    for slice_name in slices_names:
        ...
        createTrainingDataForMatchNoMatchMatchabilityComparison(mnm_path, original_live_db_path, images_path, mnm_base_source_code_dir, pairs_limit, matchornomatch_tool="matchornomatch_train_800_features_sift")
